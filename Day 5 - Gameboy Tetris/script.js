...
function draw() {
    ...
    noFill();
    stroke(colorLight);
    rect(460, 65, 120, 25, 5, 5);

    stroke(colorLight);
    rect(460, 410, 120, 120, 5, 5);

    rect(460, 215, 120, 50, 5, 5);

    rect(460, 285, 120, 50, 5, 5);
    ...
    text("Controls:", 30, 60);
    textSize(14);
    text("← → : Move side to side", 30, 90);
    text("↑ : Rotate", 30, 120);
    text("↓ : Fall faster", 30, 150);
    text("R : Reset game", 30, 180);
...
class PlayPiece {
    ...
    // Removed duplicate resetPiece. Only keep the version with forceSpawn argument below.
    ...
    futureCollision(x, y, rotation) {
        ...
        for (let i = 0; i < this.pieces.length; i++){
           if(points){
                xx = this.pos.x + x + points[i][0] * gridSpace;
                yy = this.pos.y + y + points[i][1] * gridSpace;
           }else{
                xx = this.pieces[i].pos.x + x;
                yy = this.pieces[i].pos.y + y;
           }
         // Chỉ kiểm tra va chạm với cạnh trái, phải, dưới. Không kiểm tra với cạnh trên (yy < 0)
         if(xx < gameEdgeLeft || xx + gridSpace > gameEdgeRight || yy + gridSpace > height){
             return true;
         }
         // Không kiểm tra va chạm với các khối đã tồn tại nếu yy < 0 (khối mới xuất hiện)
         if(yy < 0) continue;
    ...
    resetPiece(forceSpawn = false){
        this.rotation = 0;
        this.fallen = false;
        this.pos.x = gameEdgeLeft + 4 * gridSpace;
        this.pos.y = 0;
        this.pieceType = this.nextPieceType;
        this.generateNextPieces();
        this.newPoints();
        // Chỉ kiểm tra game over nếu không phải reset hoàn toàn
        if (!forceSpawn && gridPieces.length > 0) {
            for (let i = 0; i < this.pieces.length; i++) {
                for (let j = 0; j < gridPieces.length; j++) {
                    if (this.pieces[i].pos.x === gridPieces[j].pos.x && this.pieces[i].pos.y === gridPieces[j].pos.y) {
                        pauseGame = true;
                        gameOver = true;
                        return;
                    }
